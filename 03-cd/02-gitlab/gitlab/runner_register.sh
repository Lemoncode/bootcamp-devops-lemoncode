...
echo "Configure concurrency"
docker exec $runner_id sed -i 's/^concurrent = 1/concurrent = 4/' /etc/gitlab-runner/config.toml
echo "Restart gitlab runner to apply changes"
docker restart $runner_id
