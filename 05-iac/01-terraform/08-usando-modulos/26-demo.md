...
If we go back to the browser, we can see the current version is 6.6.0. So let's go ahead and pin it to 6.6.0 for now. We'll set the version = to 6.6.0, and this way it will only use that version until we change this argument. 
...
For the `public_subnets`, we are going to need to calculate a list of public subnet CIDR ranges for our public subnets. Let's look at how we've done this already. 

If we scroll down to our subnets, we compute the CIDR block using the cidrsubnet function and the `count.index` since we're creating our subnets and accounts. We're no longer generating our subnets in account, so we need an alternate way to generate this list of CIDR subnets. 
