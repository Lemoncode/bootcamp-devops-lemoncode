...
You set the standard for using Terraform at your company. In particular, you chose to use the VPC module from the public Terraform registry. So ideally, you'd like to import the resources created by the CloudFormation template into the VPC module. Any resources not in the VPC module can be dynamically generated by Terraform. In the `network_config_start` directory, we have a basic configuration that uses the VPC module. 

Before we make any changes, let's make a copy of the `network_config_start` directory in the root of the exercise files. We'll start by moving up to the root directory. And then depending if you're using PowerShell or BASH, run the appropriate command. This will copy the `network_config_start` into a directory called `network_config`. 
